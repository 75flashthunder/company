	...
	*/
	function _keydownhandler(e)
	{
		...
		switch (e.keyCode) {
			...
			case 13: // return
				//when user input minutes, show what user input.
				if(!self.data('timepicker-settings').validSelectValue){
					var input = self.context.value;
					if((input.length >= 4)&&(input.charAt(input.length-1) != "0")){
	...
	function _time2int(timeString)
	{
		...
		if (timeString.indexOf(":") === -1) {
			...
			if (!time) {
			    ...
			    time = timeString.match(/^([0-1]?[0-9]|2[0-3]):?([0-5][0-9])?:?([0-5][0-9])?\s*([pa]?)m?$/);
			}
		} else {
